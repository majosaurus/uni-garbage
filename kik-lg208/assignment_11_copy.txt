...
# append new line character to the end of the line
for i in range(len(raw)):
       if (re.search(end_punct, raw[i]) or re.search(end_br, raw[i])) and not re.search(abbr, raw[i]):
            raw[i] = raw[i] + "\n"


# make sentence per line list in list:
sentence_per_line = []
sent = []
for part in raw:
    if part[-1:] != "\n":
       ...
       # sent.append(" ")
        ...
        sent.append(part[:-1])
        sentence_per_line.append(sent)
        sent = []

print(sentence_per_line)
...
for lst in sentence_per_line:
    for tok in lst:
        if re.search(end_punct, tok) and len(tok) > 1 and not re.search(abbr, tok):
        ...
        elif re.search(r'\,$', tok) and len(tok) > 1: # did this because of inserting
            where = lst.index(tok)
            token1 = tok[:-1]
            end = tok[-1]
            lst.remove(tok)
            lst.insert(where, token1)
            lst.insert(where+1, end)


        elif re.search(end_br, tok) and len(tok) > 1:
            token1 = tok[:-2]
            end1 = tok[-2]
            end2 = tok[-1]
            lst.remove(tok)
            ...
            brq = tok[0]
            lst.remove(tok)
            lst.insert(where, brq) # we first want to append the front bracket or quotation
            lst.insert(where+1, token2)
            ...
            lst.remove(tok)
            lst.insert(where, token1)
            lst.insert(where+1, brq)
            

print(sentence_per_line)
-----
# make sentence as a whole in lists of list
sent_str = ""
counter = 0
for sentence in sentence_per_line:
    for element in sentence:
        sent_str += element
    sentence.insert(0, sent_str)
    del sentence[1:]
    sent_str = ""

print(sentence_per_line, "after all")


for lst in sentence_per_line:
    for se in lst:
        print(re.split(r'\b(?:[A-Za-z]\.)+|\b(?:[A-Z][a-z]{1,2}\.)+|(?:[A-Z]\.)+', se))
...
for sentence in sent:
    ...
    sentence_per_line.append(sentence)

print(sentence_per_line)

#print(re.findall(abbr, raw))
#print(re.findall(numbers, raw))




#print(re.findall(sent, raw))
#print(re.search(sent, raw))
